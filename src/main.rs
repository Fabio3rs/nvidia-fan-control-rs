    ...
    loop {
        if let Ok(gpu_temp) = nvidia_getattribute("[gpu:0]/GPUCoreTemp") {
            if last_temp != gpu_temp {
                ...
                if let Err(x) = nvidia_setattribute(
                    "[fan:0]/GPUTargetFanSpeed",
                    &(target_fan_speed as u32).to_string(),
                ) {
                    println!("[fan:0]/GPUTargetFanSpeed error {}", x);
                    break;
                }
                println!(
                    "[gpu:0]/GPUCoreTemp               {}  [fan:0]/GPUTargetFanSpeed {}",
                    gpu_temp, target_fan_speed as u32
                );
            }
        } else {
